...
Bootstrap contract (docs/explanation/bootstrap.rst): an action that uses the ci_infrastructure package nests
...
def test_action_bootstraps_exactly_when_it_needs_the_package(path: Path) -> None:
    ...
    if needs and not nests:
        pytest.fail(
            ...
            "require (see the invariant in docs/explanation/bootstrap.rst)"
        )
