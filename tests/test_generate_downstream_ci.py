...
def test_real_playground_round_trips(tmp_path: Path) -> None:
    """The 8 real manifests must validate and the generator must be idempotent.

    Each side is split by lane into two files: the runner lane keeps the unsuffixed
    cross-repo-trigger.yml / trigger-downstream.yml, the hpc lane gets the `-hpc`
    siblings. A lane with no runnable/consumer content renders None and must have no
    file on disk. The legacy `transitive-consumers.json` and pre-rename
    `triggered-by-upstream.yml` files are absent everywhere.
    """
    del tmp_path  # unused
    repo_root = Path(__file__).resolve().parents[2]
    manifests = [parse_manifest(p) for p in discover_manifests(repo_root)]
    assert len(manifests) == 8, f"expected 8 manifests, got {len(manifests)}"
    ...
    lanes = {EXECUTION_RUNNER: "", EXECUTION_HPC: "-hpc"}
    for m in manifests:
        wf_dir = m.repo_root / ".github" / "workflows"
        for lane, suffix in lanes.items():
            rendered = render_workflow(m, by_pkg, lane=lane)
            out = wf_dir / f"cross-repo-trigger{suffix}.yml"
            if rendered is None:
                assert not out.exists(), f"{m.repo} should NOT have {out.name} but does"
            else:
                assert out.exists(), f"{m.repo} should have {out.name} but doesn't"
                assert out.read_text() == rendered, f"{out} differs from generator output (regenerate?)"

            orch = render_orchestrator_workflow(m, by_pkg, by_repo, closures, lane=lane)
            orch_path = wf_dir / f"trigger-downstream{suffix}.yml"
            if orch is None:
                assert not orch_path.exists(), f"{m.repo} should NOT have {orch_path.name} but does"
            else:
                assert orch_path.exists(), f"{m.repo} should have {orch_path.name} but doesn't"
                assert orch_path.read_text() == orch, f"{orch_path} differs from generator output (regenerate?)"

        json_path = m.repo_root / ".ci" / "transitive-consumers.json"
        assert not json_path.exists(), f"{m.repo} still has the legacy {json_path.name}"
        legacy = wf_dir / "triggered-by-upstream.yml"
        assert not legacy.exists(), f"{m.repo} still has the pre-rename {legacy.name}"


def test_real_playground_closure_shape() -> None:
    """Sanity-check the actual closure values for the 6-repo setup.

    fortran -> {cxx, cxx-py, ecflow, py}; cxx-py is a transitive consumer
    even though fortran's [[trigger-downstream]] only lists cxx directly --
    that's the whole point of this redesign.
    """
    repo_root = Path(__file__).resolve().parents[2]
    manifests = [parse_manifest(p) for p in discover_manifests(repo_root)]
    closures = compute_transitive_consumers(manifests)

    fortran_repo = "ecmwf-enterprise-sandbox/downstream-ci-repo-fortran"
    fortran_closure = closures[fortran_repo]
    assert "fortmath/build" in fortran_closure
    entry = fortran_closure["fortmath/build"]
    assert set(entry["consumers"]) == {
        "ecmwf-enterprise-sandbox/downstream-ci-repo-cxx",
        "ecmwf-enterprise-sandbox/downstream-ci-repo-cxx-py",
        "ecmwf-enterprise-sandbox/downstream-ci-repo-ecflow",
        "ecmwf-enterprise-sandbox/downstream-ci-repo-py",
    }
    # Every transitive consumer's runnable kind that depends on fortmath/build
    # should appear as a caller-side job in the orchestrator. clang-tidy is
    # intentionally has no triggers in any consumer manifest, so it's
    # absent here -- direct pushes to each consumer's own repo still run it.
    # cxx, cxx-py and ecflow merge build+test into a single `build` kind (ecflow
    # also has build-python); only py keeps a standalone `test` kind.
    assert set(entry["expected-checks"]) == {
        "cxxmath/build",
        "cxxmath-python/build",
        "ecflowmath/build",
        "ecflowmath/build-python",
        "pymath/test",
    }
